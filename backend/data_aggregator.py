...
class CS2DataAggregator:
    ...
    def _combine_data(self, hltv_matches: List[Dict], prizepicks_props: List[Dict], underdog_props: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        ...
        for hltv_match in hltv_matches:
            ...
            # Create match object for ALL matches (with or without props)
            match = {
                'id': match_id,
                'team1': hltv_match['team1'],
                'team2': hltv_match['team2'],
                ...
                'map2': 'TBD',
                'status': 'upcoming',
                'has_props': len(raw_props) > 0  # Flag to indicate if props are available
            }
            matches.append(match)
            
            if raw_props:
                matches_with_props += 1
                ...
                logger.info(f"📊 Match {hltv_match['team1']} vs {hltv_match['team2']} - no PrizePicks props available")
        
        logger.info(f"✅ Showing all {len(matches)} matches ({matches_with_props} with props, {len(matches) - matches_with_props} without props)")
